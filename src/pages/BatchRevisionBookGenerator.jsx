                                ...
                                <div className="space-y-2 max-h-96 overflow-y-auto">
                                    {generatedBooks.map((book, index) => (
                                        <div key={index} className="flex items-center gap-3 p-3 bg-slate-50 rounded-lg">
                                            {book.status === 'success' && <CheckCircle2 className="w-5 h-5 text-green-500 flex-shrink-0" />}
                                            {book.status === 'skipped' && <AlertCircle className="w-5 h-5 text-blue-500 flex-shrink-0" />}
                                            {book.status === 'retrying' && <Loader2 className="w-5 h-5 text-amber-500 flex-shrink-0 animate-spin" />}
                                            {book.status === 'error' && <AlertCircle className="w-5 h-5 text-red-500 flex-shrink-0" />}
